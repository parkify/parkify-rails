class Api::V2::AccountController < ApplicationController
  ...
  def show
    ...
    respond_to do |format|
      #format.html # show.html.erb
        format.json { render json: {:user=>Api::V2::UsersPresenter.new().as_json(@user.as_json), :success=>true } }
    end
  end
  ...
  def new
    ...
    respond_to do |format|
      ...
      format.json { render json: Api::V2::UsersPresenter.new().as_json(@user.as_json) }
    end
  end
    ...
    respond_to do |format|
      if @user.save
        #format.html { redirect_to @user, notice: 'User was successfully created.' }
          format.json { render json: Api::V2::UsersPresenter.new().as_json(@user.as_json), status: :created, location: @user }
      else
  ...
  def update
    ...
    respond_to do |format|
      ...
      if @user.update_attributes(JSON.parse(params[:user]))
        ...
        format.json { render json: {:user=>Api::V2::UsersPresenter.new().as_json(@user.as_json), :success=>true}, location: @user }
      else
        puts "3"
  ...
  def update_password
    ...
    respond_to do |format|
      if @user.update_attributes(JSON.parse(params[:user]))
        ...
        format.json { render json: {:user=>Api::V2::UsersPresenter.new().as_json(@user.as_json), :auth_token=>@user.authentication_token, :success=>true}, location: @user }
      else
        p @user.errors
          format.json { render json: {:error=>@user.errors}, status: :unprocessable_entity }
      end
