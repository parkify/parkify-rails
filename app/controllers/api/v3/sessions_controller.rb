class Api::V3::SessionsController < ApplicationController
  ...
  def create
    ...
    if resource.valid_password?(user_login["password"])
      ...
      render :json=> {:success=>true, :user=>Api::V3::UsersPresenter.new().as_json(@user), :auth_token=>resource.authentication_token, :license_plate_number => lpn, :last_four_digits => last_four_digits }
      return
    end
