class ResourceOfferContainer 
  ...
  def start_time(time=Time.now(), total=false)
    ...
    if(capIntervals.size == 0)
      p ["no cap Intervals in ResourceOfferContainer::start_time", time, total, thisarray]
      return Time.at(1)
    end
  ...
  def validate_reservation(start_time, end_time)
    ...
    #DEBUG: start
    if(!valid)
      p [["invalid time in ResourceOfferContainer::validate_reservation", "t_in:", start_time, end_time, "t_my:", self.start_time(start_time), self.end_time(start_time)], self]
    end
    #DEBUG: end
    return valid
  end
